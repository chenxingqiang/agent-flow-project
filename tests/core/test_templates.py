...
async def test_outlier_removal_template():
    ...
    assert "data" in result["step-1"]
    assert isinstance(result["step-1"]["data"], np.ndarray)
    assert result["step-1"]["data"].shape[1] == data.shape[1]  # Same number of features
    assert result["step-1"]["data"].shape[0] <= data.shape[0]  # Some points may be removed as outliers
