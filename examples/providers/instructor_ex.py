"""
The following example shows how to implement your own provider to use ell2a with instructor.
These type of changes won't be added to ell2a but you can use this as a starting point to
...
class InstructorProvider(OpenAIProvider):
    def translate_to_provider(self, *args, **kwargs):
        """ This translates ell2a call parameters to the provider call parameters. In this case instructor is just an openai client, 
    ...
    def translate_from_provider(self,provider_response,
        ...
        """This translates the provider response (the result of calling client.chat.completions.create with the parameters from translate_to_provider)
          to an an ell2a message. In this case instructor just returns a pydantic type which we can use to create an ell2a response model. """
...
# We then register the provider with ell2a. We will use InstructorProvider any time an instructor.Instructor type client is used.
ell2a.register_provider(InstructorProvider(), instructor.Instructor)
...
@ell2a.complex(model="gpt-4-turbo-preview", client=client, response_model=UserDetail)
