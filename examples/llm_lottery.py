...
@ell2a.complex(model="gpt-4o-mini", tools=[buy_lottery_ticket, check_lottery_result])
def play_lottery(message_history: List[Message]) -> List[Message]:
    ...
    return [
        ell2a.system("You are an AI assistant that plays the lottery. Buy a lottery ticket with a number between 0 and 16, then check if it wins. If it doesn't win, try again with a different number.")
    ] + message_history
...
def main():
    ...
    message_history = []
    message_history.append(ell2a.user("Let's play the lottery until we win!"))
